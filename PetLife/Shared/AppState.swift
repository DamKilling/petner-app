...
actor InMemoryPetBackend {
    ...
    init() {
        ...
        self.feedPosts = [
            FeedPost(
                ...
                topic: "同城交友",
                city: "上海",
                content: "想给我家英短找固定玩伴，周末徐汇滨江可以一起遛猫背包，也欢迎先交换宠物档案。",
                tags: ["英短", "徐汇", "周末"],
                likes: 128,
                comments: [
        ...
        self.chatThreads = [
            ChatThread(
                ...
                messages: [
                    ...
                    ChatMessage(id: UUID(), text: "太好了，我今晚把 Seven 的档案卡发你。", isFromCurrentUser: false, sentAtText: "今天 08:05")
                ]
            )
        ]
    }
    ...
    func openChat(for pet: PetProfile) async -> (BootstrapPayload, UUID) {
        if let existing = chatThreads.first(where: { $0.title.contains(pet.ownerName) || $0.title.contains(pet.name) }) {
